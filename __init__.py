...
class Message:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.suppress_autogenerated_header = kwargs.pop("suppress_autogenerated_header", False)
        ...
        if not self.suppress_autogenerated_header:
    ...
    def __str__(self):
        to = self.to if isinstance(self.to, str) else ", ".join(self.to)
        return f"{escape(to)} : {escape(self.subject)}"
    ...
    def send(self, **kwargs):
        ...
        if log.isEnabledFor(logging.INFO):
            to_str = to_emails if isinstance(to_emails, str) else ", ".join(to_emails)
            header_pairs = [
                ("From", self.from_email),
                ("To", to_str),
                ("Subject", self.subject),
            ]
            if getattr(self, "cc", None):
                cc = self.cc if isinstance(self.cc, str) else ", ".join(self.cc)
                header_pairs.append(("Cc", cc))
            if getattr(self, "bcc", None):
                bcc = self.bcc if isinstance(self.bcc, str) else ", ".join(self.bcc)
                header_pairs.append(("Bcc", bcc))
            if getattr(self, "Replyto", None):
                header_pairs.append(("Reply-To", self.Replyto))
            for hdr_attr in ("init_headers", "extra_headers"):
                for name, value in (getattr(self, hdr_attr, None) or {}).items():
                    header_pairs.append((name, value))
            seen = set()
            deduped = []
            for name, value in header_pairs:
                key = (name.lower(), str(value))
                if key in seen:
                    continue
                seen.add(key)
                deduped.append(f"{name}: {value}")
            body = self.Body or (markdownify(self.Html) if self.Html else "")
            # log.info("EMAILER message (RFC 822):\n" + "\n".join(deduped) + "\n\n" + body)
